...
pub fn parse_spl_token_account(bytes: &[u8]) -> Result<TokenAccountView<'_>> {
    ...
    let mint = bytes[0..32].try_into()
        .map_err(|_| ScoutError::Decode { 
            field: "mint".to_string(), 
            expected: "32 bytes".to_string(), 
            got: format!("{} bytes", bytes.len().min(32)) 
        })?;
    
    let owner = bytes[32..64].try_into()
        .map_err(|_| ScoutError::Decode { 
    ...
    let amount = u64::from_le_bytes(bytes[64..72].try_into()
        .map_err(|_| ScoutError::Decode { 
    ...
    let is_native = if is_native_option == 1 && bytes.len() >= 115 {
        Some(u64::from_le_bytes(bytes[107..115].try_into()
            .map_err(|_| ScoutError::Decode { 
    ...
    let delegated_amount = if bytes.len() >= 123 {
        u64::from_le_bytes(bytes[115..123].try_into()
            .map_err(|_| ScoutError::Decode { 
...
mod tests {
    ...
    fn test_parse_too_short() {
        ...
        assert!(matches!(result, Err(ScoutError::DataTooShort { expected: 72, got: 50 })));
    }
}
