...
impl TokenRow {
    pub fn from_view(account: String, view: &TokenAccountView<'_>) -> Self {
        Self {
            ...
            close_authority: view
                .close_authority
                .map(|ca| bs58::encode(ca).into_string()),
        }
    }
}
...
pub fn print_summary(
    sol: f64,
    _tokens: &[TokenRow],
    insights: &crate::analysis::WalletInsights,
) -> anyhow::Result<()> {
    println!("┌─────────────────────────────────────────────────────────────────────────────────┐");
...
pub fn print_debug(view: &TokenAccountView<'_>) {
    ...
    println!(
        "  Mint (0..32):           {}",
        hex::encode(&view.raw[0..32.min(view.raw.len())])
    );
    println!(
        "  Owner (32..64):         {}",
        hex::encode(&view.raw[32..64.min(view.raw.len())])
    );
    println!(
        "  Amount (64..72):        {}",
        hex::encode(&view.raw[64..72.min(view.raw.len())])
    );
    ...
    if view.raw.len() > 115 {
        println!(
            "  Delegated Amount (115..123): {}",
            hex::encode(&view.raw[115..123.min(view.raw.len())])
        );
    }

    if view.raw.len() > 123 {
        println!(
            "  Close Authority Option (123): {:02x}",
            view.close_authority_option
        );
    }
    ...
    if let Some(close_authority) = view.close_authority {
        println!(
            "  Close Authority: {}",
            bs58::encode(close_authority).into_string()
        );
    }
}
