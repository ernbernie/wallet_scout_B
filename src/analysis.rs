use crate::patterns::{
    AirdropWalletPattern, DistributionWalletPattern, ExchangeWalletPattern, HighValueWalletPattern,
...
impl WalletAnalyzer {
    ...
    pub fn analyze(&self, tokens: &[TokenRow]) -> WalletInsights {
        ...
        let recommendations =
            self.generate_recommendations(&wallet_type, &risk_level, &detected_patterns);
    ...
    fn calculate_statistics(&self, tokens: &[TokenRow]) -> WalletStatistics {
        ...
        let small_accounts = tokens
            .iter()
            .filter(|t| t.amount_raw > 0 && t.amount_raw < 1_000_000)
            .count();
        let medium_accounts = tokens
            .iter()
            .filter(|t| t.amount_raw >= 1_000_000 && t.amount_raw < 1_000_000_000)
            .count();
        let large_accounts = tokens
            .iter()
            .filter(|t| t.amount_raw >= 1_000_000_000)
            .count();
    ...
    fn generate_recommendations(
        ...
        match risk_level {
            ...
            RiskLevel::High => {
                recommendations.push(
                    "🔴 HIGH RISK: This wallet shows high-risk patterns. Monitor closely."
                        .to_string(),
                );
            }
            RiskLevel::Medium => {
        ...
        match wallet_type {
            ...
            WalletType::Suspicious => {
                recommendations.push(
                    "🚨 Suspicious activity detected. Avoid interacting with this wallet."
                        .to_string(),
                );
            }
            _ => {}
        ...
        if patterns.iter().any(|p| p.name == "High Value Wallet") {
            recommendations
                .push("💰 High-value wallet detected. Consider security implications.".to_string());
        }
