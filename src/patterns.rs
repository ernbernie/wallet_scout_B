...
impl Pattern for DistributionWalletPattern {
    ...
    fn detect(&self, tokens: &[TokenRow]) -> Option<PatternMatch> {
        ...
        let variance = if tokens.len() > 1 {
            ...
            let sum_squared_diff: f64 = amounts.iter().map(|&x| (x as f64 - mean).powi(2)).sum();
        ...
        if has_varying_amounts && has_large_amounts {
            ...
            features.insert(
                ...
                *amounts.iter().max().unwrap_or(&0) as f64,
            );

            let mut counts = BTreeMap::new();
            counts.insert("total_accounts".to_string(), tokens.len());
            counts.insert(
                "large_amount_accounts".to_string(),
                amounts.iter().filter(|&&x| x > 1_000_000_000).count(),
            );
...
impl Pattern for AirdropWalletPattern {
    ...
    fn detect(&self, tokens: &[TokenRow]) -> Option<PatternMatch> {
        ...
        if small_ratio > 0.7 || very_small_ratio > 0.5 || one_token_accounts > 10 {
            let confidence = (small_ratio * 0.5
                + very_small_ratio * 0.3
                + (one_token_accounts as f64 / tokens.len() as f64) * 0.2)
                .min(1.0);
            let evidence = vec![
                format!("{} accounts with same mint", tokens.len()),
                format!(
                    "{}% small amounts (< 100M tokens)",
                    (small_ratio * 100.0) as u32
                ),
...
impl Pattern for ExchangeWalletPattern {
    ...
    fn detect(&self, tokens: &[TokenRow]) -> Option<PatternMatch> {
        ...
        if mint_diversity > 0.3 && unique_mints.len() > 5 {
            let confidence =
                (mint_diversity * 0.7 + (unique_mints.len() as f64 / 100.0).min(0.3)).min(1.0);
            let evidence = vec![
                format!(
                    "{} unique mints out of {} accounts",
                    unique_mints.len(),
                    tokens.len()
                ),
    ...
    fn detect(&self, tokens: &[TokenRow]) -> Option<PatternMatch> {
        ...
        if tokens.len() > 500 {
            ...
            if same_mint {
                suspicious_indicators
                    .push(format!("{} accounts with identical mint", tokens.len()));
            }
        }
        ...
        if empty_ratio > 0.8 && tokens.len() > 100 {
            suspicious_indicators.push(format!(
                "{}% empty accounts ({} out of {})",
                (empty_ratio * 100.0) as u32,
                empty_accounts,
                tokens.len()
            ));
        }
        ...
        if suspicious_indicators.len() >= 2 {
            ...
            features.insert(
                ...
                suspicious_indicators.len() as f64,
            );
...
impl Pattern for HighValueWalletPattern {
    ...
    fn detect(&self, tokens: &[TokenRow]) -> Option<PatternMatch> {
        ...
        let high_individual_accounts = tokens
            .iter()
            .filter(|t| t.amount_raw > 1_000_000_000_000)
            .count();
        ...
        if high_total_value || high_individual_accounts > 0 {
            ...
            let mut thresholds = BTreeMap::new();
            thresholds.insert(
                "high_total_value_threshold".to_string(),
            ...
            features.insert(
                "high_individual_accounts".to_string(),
                high_individual_accounts as f64,
            );
