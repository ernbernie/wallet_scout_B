...
fn test_distribution_wallet_pattern() {
    ...
    assert!(
        result.is_some(),
        "Should detect distribution wallet pattern"
    );
...
fn test_distribution_wallet_pattern_small_wallet() {
    ...
    assert!(
        result.is_none(),
        "Should not detect distribution pattern for small wallet"
    );
}
...
fn test_exchange_wallet_pattern() {
    ...
    assert!(pattern_match
        .evidence
        .iter()
        .any(|e| e.contains("20 unique mints")));
}
...
fn test_suspicious_activity_pattern() {
    ...
    assert!(
        result.is_some(),
        "Should detect suspicious activity pattern"
    );
    ...
    assert!(pattern_match
        .evidence
        .iter()
        .any(|e| e.contains("1500 accounts")));
}
...
fn test_pattern_detection_empty_wallet() {
    ...
    for pattern in patterns {
        ...
        assert!(
            result.is_none(),
            "Should not detect any patterns in empty wallet"
        );
    }
}
...
fn test_pattern_detection_single_account() {
    ...
    for pattern in patterns {
        ...
        assert!(
            result.is_none(),
            "Should not detect patterns with single account"
        );
    }
}
