...
fn test_analysis_boundary_conditions() {
    ...
    for i in 0..50 {  // Exactly 50 accounts (minimum for distribution)
        tokens.push(TokenRow {
...
fn test_analysis_malformed_data() {
    ...
    for i in 0..10 {
        tokens.push(TokenRow {
            account: if i % 2 == 0 { "".to_string() } else { format!("Account{}", i) },
            mint: if i % 3 == 0 { "".to_string() } else { format!("Mint{}", i) },
            owner: if i % 4 == 0 { "".to_string() } else { format!("Owner{}", i) },
            amount_raw: if i % 5 == 0 { 1_000_000_000_000 } else { i as u64 * 1_000_000 },
            state: if i % 6 == 0 { 255 } else { 0 },
            delegated_amount: if i % 7 == 0 { 1_000_000_000_000 } else { 0 },
            delegate: if i % 8 == 0 { Some("".to_string()) } else { None },
            close_authority: if i % 9 == 0 { Some("".to_string()) } else { None },
        });
    }
